...
;;;; As toki pona has uniform pronounciation rules, we can generate that
;;;; part on the fly.
...
;;; There are cases in toki pona where one word has multiple
;;; grammatical functions so we merge them into one dictionary entry.
...
(defun add-word-fun (spell fn)
  ...
  (let* ((old-classes (agm:get-word spell))
	 (new-classes (if old-classes
			  (push fn old-classes)
			  (list fn))))
    (agm:put-word spell new-classes)))

;;; Generate a Julius VOCA file from our own WORDS file.
(defun make-voca (fname)
  ...
  (let (;; Input file
    ...
    (labels
	;; Extract spelling and phonetics.
	((modify-spelling (class spell)
	 ...
	 until (eq line NIL) do
	   (cond
	     ...
	     ;; Ignore everything else, like blank lines.
	     (T T))))
